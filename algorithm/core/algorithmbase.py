...
from algorithm.core.criterions import CELoss, ConsistencyLoss

from algorithm.core.hooks import (
    AimHook,
    CheckpointHook,
    DistSamplerSeedHook,
    EMAHook,
...
from algorithm.core.utils import (
    Bn_Controller,
...
class AlgorithmBase:
    ...
    def __init__(self, args, net_builder, tb_log=None, logger=None, **kwargs):
        ...
        self.class_entropy = torch.zeros(self.num_classes).cuda()
        self.class_entropy_t = torch.ones(self.num_classes).cuda()
        self.num_samples = torch.zeros(self.num_classes).cuda()
        self.prob_max_mu = 0
        self.prob_max_var = 1
        self.m = 0.999
    ...
    def update_class(self):
        cond = self.num_samples > 0
        self.class_entropy_t[cond] = self.class_entropy_t[cond] / self.num_samples[cond]
        self.class_entropy_t[cond] = self.class_entropy_t[cond] / torch.max(self.class_entropy_t[cond])        
        self.class_entropy_t[cond] = self.class_entropy_t[cond] - torch.mean(self.class_entropy_t[cond])
        self.class_entropy[cond] = 0.999 * self.class_entropy[cond] + 0.001 * self.class_entropy_t[cond]        
        self.num_samples = torch.zeros(self.num_classes).cuda()
        self.class_entropy_t = torch.zeros(self.num_classes).cuda()
        return
    ...
    def evaluate(self, eval_dest="eval", out_key="logits", return_logits=False):
        ...
        with torch.no_grad():
            for data in eval_loader:
                ...
                top5_correct += torch.sum(top5_preds.eq(y.view(-1, 1).expand_as(top5_preds)).any(dim=1)).item()
        ...
        eval_dict = {
            ...
            eval_dest + "/F1": F1,
            eval_dest + "/top-5-acc": top5_correct / total_num,
        }
        if return_logits:
    ...
    def get_save_dict(self):
        ...
        save_dict = {
            ...
            "class_entropy": self.class_entropy,
            "prob_max_mu": self.prob_max_mu,
            "prob_max_var": self.prob_max_var, 
        }
    ...
    def load_model(self, load_path):
        ...
        self.class_entropy = checkpoint["class_entropy"]
        self.class_entropy = self.class_entropy.cuda(self.gpu)
        self.prob_max_mu = checkpoint["prob_max_mu"]
        self.prob_max_var = checkpoint["prob_max_var"]
