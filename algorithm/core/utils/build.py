...
from algorithm.datasets import get_collactor, name2sampler
from algorithm.nets.utils import param_groups_layer_decay, param_groups_weight_decay
...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    """
    from algorithm.datasets import get_eurosat, get_medmnist, get_semi_aves, get_cifar, get_svhn, get_stl10, get_imagenet
...
def get_data_loader(args,
    ...
    if isinstance(data_sampler, str):
        ...
        data_loader = DataLoader(dset, batch_size=batch_size, shuffle=False, num_workers=num_workers, collate_fn=collact_fn,
        ...
        data_loader = DataLoader(dset, batch_size=batch_size, shuffle=False, num_workers=num_workers,
...
def get_cosine_schedule_with_warmup(optimizer,
    ...
    def _lr_lambda(current_step):
        ...
        '''
        # 将num_warmup_steps改为10240
        if current_step < 0:
                _lr = float(current_step) / float(max(1, 0))

        else:
            num_cos_steps = float(current_step - 0)
            num_cos_steps = num_cos_steps / float(max(1, num_training_steps - 0))
