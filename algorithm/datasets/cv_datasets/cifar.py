...
from algorithm.datasets.augmentation import RandAugment, RandomResizedCropAndInterpolation
from algorithm.datasets.utils import split_ssl_data
...
def get_cifar(args, alg, name, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True):
    ...
    transform_ws = transforms.Compose([
        transforms.Resize(crop_size),
        transforms.RandomCrop(crop_size, padding=int(crop_size * (1 - crop_ratio)), padding_mode='reflect'),
        transforms.RandomHorizontalFlip(),
        transforms.RandomApply([RandAugment(3, 5)], p=0.0),
        transforms.ToTensor(),
        transforms.Normalize(mean[name], std[name])
    ])
    ...
    lb_dset = BasicDataset(alg, lb_data, lb_targets, num_classes, transform_weak, False, transform_ws, False)
